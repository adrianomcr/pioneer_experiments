...
# Callback to get the pose of the robot
def callback_pose_pioneer(data):
    global pos, rpy
    ...
    rpy[2] = data.pose.pose.orientation.x # TEMPORARIO !!!!!!!!!!!!!!!!!!!!!!!!!!! ISSO NAO EH UM QUATERNIO
...
def vec_field(pos):
    ...
    Vx = v_r*( G*grad_D[0] + H*T[0] )
    Vy = v_r*( G*grad_D[1] + H*T[1] )
...
def vector_field():
    ...
    rospy.Subscriber("/xsens/pose", Odometry, callback_pose_pioneer) # ground thruth
    # rospy.Subscriber("/decawave/pose", Odometry, callback_pose_pioneer) # ground thruth
...
if __name__ == '__main__':
    ...
    v_r = float(sys.argv[1]) # forward velocity
